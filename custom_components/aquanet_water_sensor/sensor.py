...
async def async_setup_platform(
    ...
    for x in pgps.ppg_list:
        async_add_entities(
            [AquanetSensor(hass, api, x.meter_number, x.id_local),
             AquanetInvoiceSensor(hass, api, x.meter_number, x.id_local),
             AquanetCostTrackingSensor(hass, api, x.meter_number, x.id_local)],
            update_before_add=True)


class AquanetSensor(SensorEntity):
    def __init__(self, hass, api: AquanetApi, meter_id: string, id_local: int) -> None:
        ...
        self.entity_name = "Aquanet Water Sensor " + meter_id + " " + str(id_local)
...
class AquanetInvoiceSensor(SensorEntity):
    def __init__(self, hass, api: AquanetApi, meter_id: string, id_local: int) -> None:
        ...
        self.meter_id = meter_id
        self.id_local = id_local
        self.entity_name = "Aquanet Water Invoice Sensor " + meter_id + " " + str(id_local)

    @property
    def unique_id(self) -> str | None:
        return "aquanet_invoice_sensor" + self.meter_id + "_" + str(self.id_local)
...
class AquanetCostTrackingSensor(SensorEntity):
    def __init__(self, hass, api: AquanetApi, meter_id: string, id_local: int) -> None:
        ...
        self.meter_id = meter_id
        self.id_local = id_local
        self.entity_name = "Aquanet Water Cost Tracking Sensor " + meter_id + " " + str(id_local)

    @property
    def unique_id(self) -> str | None:
        return "aquanet_cost_tracking_sensor" + self.meter_id + "_" + str(self.id_local)
